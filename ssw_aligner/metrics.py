...
from ssw_aligner.alignment_utils import get_cigar_op_groups, unfold_cigar
...
def compute_raw_score_aa(
    ...
    substitution_matrix: Mapping[str, Mapping[str, float]] = BLOSUM_62,
) -> float:
    raw_score = 0.0
    query_pos = 0
    target_pos = 0
...
def compute_bit_score(raw_score: int | float, gumbell_params: GumbellParams = GumbellParams.IGBLAST) -> float:
    return (gumbell_params.value["L"] * raw_score - math.log(gumbell_params.value["K"])) / math.log(2)


def compute_raw_score_from_bit_score(
    bit_score: float, gumbell_params: GumbellParams = GumbellParams.IGBLAST
) -> float:
...
def calculate_seq_identity(
    cigar: str, query: str, target: str, query_start: int = 0, target_start: int = 0
) -> float:
